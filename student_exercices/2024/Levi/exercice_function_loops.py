...
def func9(text, tranlate_dict):

    # text='Good Morning', tranlate_dict={'g': 't', 'r': 'z'}

    result = ''
    for letter in text:
        if letter.lower() in tranlate_dict:
            if letter.isupper():
                result += tranlate_dict[letter.lower()].upper()
            else:
                result += tranlate_dict[letter]
        else:
            result += letter
    return result
...
if __name__ == '__main__':
    ...
    # # replaced from the value from the dict (if in the dict) else the base letter
